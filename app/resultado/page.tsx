...
const AVATAR_MAP: Record<string, Record<string, Record<string, string>>> = {
  dog: {
    pequeno: {
      claro: 'pinscher-caramelo',
      escuro: 'pinscher-preto-fogo',
      mesclado: 'chihuahua-creme',
    },
    medio: {
      claro: 'beagle',
      escuro: 'srd-medio-escuro',
      mesclado: 'border-collie',
    },
    grande: {
      claro: 'golden-retriever',
      escuro: 'labrador-preto',
      mesclado: 'pastor-alemao',
    },
  },
  cat: {
...
function Flames() {
  return (
    <div style={{position:'absolute',bottom:0,left:0,right:0,height:90,display:'flex',alignItems:'flex-end',justifyContent:'space-around',padding:'0 2px',zIndex:1}}>
      {[
        {w:30,h:80,d:'flicker1',o:0.85,t:1.2},
        {w:22,h:60,d:'flicker2',o:0.75,t:0.9},
        {w:38,h:95,d:'flicker3',o:0.9,t:1.4},
        {w:20,h:55,d:'flicker1',o:0.7,t:1.0},
        {w:32,h:75,d:'flicker2',o:0.85,t:1.3},
        {w:20,h:50,d:'flicker3',o:0.7,t:0.8},
        {w:28,h:68,d:'flicker1',o:0.8,t:1.1},
      ].map((f,i) => (
        <div key={i} style={{
          width:f.w, height:f.h,
          background:'linear-gradient(180deg,#ffcc00,#ff3300)',
          borderRadius:'50% 50% 15% 15%',
          animation:`${f.d} ${f.t}s ease-in-out infinite`,
          transformOrigin:'bottom', opacity:f.o,
        }} />
      ))}
    </div>
  )
...
function Stars() {
  return (
    <>
      {[
        {t:15,l:20},{t:25,l:80},{t:10,r:30},{t:40,l:140},{t:30,r:60},{t:55,l:50},
      ].map((s,i) => (
        <div key={i} style={{
          position:'absolute', top:s.t, left:(s as any).l, right:(s as any).r,
          width: i%2===0?4:3, height: i%2===0?4:3,
          background:'#c084fc', borderRadius:'50%', zIndex:1, opacity:0.5,
    ...
    <main style={{background:'#f9f8ff', minHeight:'100vh', padding:'2rem 1rem'}}>
      <div style={{maxWidth:380, margin:'0 auto'}}>
        ...
        <div style={{
          ...
          <div style={{borderRadius:16, overflow:'hidden', background:'#fff', border:'1.5px solid rgba(200,150,255,0.2)'}}>

            {/* HEADER */}
            <div style={{padding:'12px 16px 10px', display:'flex', alignItems:'center', justifyContent:'space-between', borderBottom:'1px solid rgba(168,85,247,0.08)'}}>
              <div style={{display:'flex', alignItems:'center', gap:7}}>
                <Image src="/logo.png" alt="SignoPet" width={24} height={24}/>
                <span style={{fontSize:12, fontWeight:600, color:'#4c1d95', fontFamily:'sans-serif'}}>signopet</span>
              </div>
              <div style={{background:cfg.badge, border:`1px solid ${cfg.badgeBorder}`, borderRadius:20, padding:'3px 10px'}}>
                <span style={{fontSize:10, color:cfg.badgeText, fontFamily:'sans-serif', fontWeight:600, letterSpacing:'0.04em'}}>
            ...
            <div style={{position:'relative', overflow:'hidden', background:cfg.avatarBg, padding:'20px 16px 0', minHeight:270}}>
              ...
              <div style={{position:'relative', zIndex:2, textAlign:'center', paddingBottom:85}}>
                ...
                <div style={{fontSize:26, fontWeight:700, color:'#1a0500', fontFamily:'serif', marginTop:10, lineHeight:1}}>{data.nome}</div>
                <div style={{fontSize:12, color:cfg.texto, fontStyle:'italic', fontFamily:'sans-serif', marginTop:3}}>{data.frase_pet}</div>
              </div>
            </div>

            {/* COMPATIBILIDADE */}
            <div style={{background:cfg.compatBg, padding:'14px 20px', textAlign:'center'}}>
              <div style={{fontSize:10, color:'rgba(255,255,255,0.5)', fontFamily:'sans-serif', letterSpacing:'0.08em', marginBottom:4, textTransform:'uppercase'}}>
                {data.nome} e você
              </div>
              <div style={{display:'flex', alignItems:'baseline', justifyContent:'center', gap:6, marginBottom:8}}>
                <span style={{fontSize:46, fontWeight:700, color:cfg.textoSub, fontFamily:'sans-serif', lineHeight:1, textShadow:`0 0 20px ${cfg.textoSub}44`}}>
                  {data.score}%
                </span>
                <span style={{fontSize:12, color:'rgba(255,255,255,0.5)', fontFamily:'sans-serif'}}>compatíveis</span>
              </div>
            ...
            {/* RECADO IRÔNICO */}
            <div style={{padding:'14px 18px', background:'#fff', borderBottom:'1px solid rgba(168,85,247,0.08)'}}>
              <div style={{fontSize:9, color:'#c084fc', fontFamily:'sans-serif', fontWeight:600, letterSpacing:'0.08em', marginBottom:6, textAlign:'center'}}>
                {data.nome.toUpperCase()} DEIXOU UM RECADO:
              </div>
              <div style={{fontSize:13, color:'#2d0a4e', fontStyle:'italic', fontFamily:'serif', textAlign:'center', lineHeight:1.6}}>
                "{data.frase_compat}"
              </div>
            </div>
            ...
            <div style={{padding:'10px 20px', background:'#fafafa', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
              <div style={{textAlign:'center'}}>
                <div style={{fontSize:8, color:'#9ca3af', fontFamily:'sans-serif', letterSpacing:'0.08em', marginBottom:2}}>PET</div>
                <div style={{fontSize:13, color:'#1a0a2e', fontFamily:'sans-serif', fontWeight:600}}>✦ {data.signo_pet}</div>
                <div style={{fontSize:9, color:cfg.texto, fontFamily:'sans-serif'}}>{cfg.emoji} {cfg.label}</div>
              </div>
              <div style={{width:1, height:28, background:'rgba(168,85,247,0.15)'}}/>
              <div style={{textAlign:'center'}}>
                <div style={{fontSize:8, color:'#9ca3af', fontFamily:'sans-serif', letterSpacing:'0.08em', marginBottom:2}}>TUTOR</div>
                <div style={{fontSize:13, color:'#1a0a2e', fontFamily:'sans-serif', fontWeight:600}}>✦ {data.signo_tutor}</div>
                <div style={{fontSize:9, color:cfg.texto, fontFamily:'sans-serif'}}>{cfg.emoji} {ELEMENTO_CONFIG[data.elemento]?.label}</div>
              </div>
            </div>

            {/* RODAPÉ */}
            <div style={{padding:'8px 20px', background:'#fff', display:'flex', alignItems:'center', justifyContent:'center', borderTop:'1px solid rgba(168,85,247,0.08)'}}>
              <span style={{fontSize:10, color:'#c4b5fd', fontFamily:'sans-serif', fontStyle:'italic', letterSpacing:'0.03em'}}>
        ...
        <button style={{
          ...
          fontWeight:700, fontSize:16, border:'none', cursor:'pointer', marginBottom:12,
        ...
        <div style={{background:'#fff', border:'1px solid #f0e0ff', borderRadius:20, padding:20, textAlign:'center'}}>
          ...
          <button style={{
            width:'100%', padding:'14px', borderRadius:999, color:'#fff',
      ...
      <style>{`
        ...
        @keyframes flicker1{0%,100%{transform:scaleY(1)}50%{transform:scaleY(1.2) translateX(1px)}}
        @keyframes flicker2{0%,100%{transform:scaleY(1)}50%{transform:scaleY(0.85) translateX(-2px)}}
        @keyframes flicker3{0%,100%{transform:scaleY(1)}40%{transform:scaleY(1.15)}}
